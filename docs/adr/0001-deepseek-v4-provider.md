...
- **Anthropic direct API** — standard choice but higher cost and regional availability constraints. The project's wrapper is provider-agnostic via `CLAUDE_DELEGATOR_MODEL`, so switching back requires only an env var change.
...
- Provide manual override through `CLAUDE_DELEGATOR_MODEL`.
...
- The wrapper defaults (`run-claude-code.sh`) contain non-standard model IDs, which may confuse first-time GitHub visitors. Mitigated by `CLAUDE_DELEGATOR_MODEL` override documentation and the model table in SKILL.md.
...
- No provider lock-in: the wrapper only references models via `--model` flag and env var; changing provider requires only `cc switch` and a `CLAUDE_DELEGATOR_MODEL` override.
